...
ncaa_park_factor <- function(id, years, type = "conference", ...) {
  ...
  y = ifelse(2020 %in% years & length(years) < 6, length(years)-1,length(years))
  ...
  if (y == 1) {
    ...
    df = df %>%
      dplyr::mutate(
        ...
        away_game = as.numeric(ifelse(.data$away_team_id == id | !is.na(.data$neutral_site), 1, 0)),
        runs_scored_home = as.numeric(ifelse(.data$home_game == 1, .data$home_team_score,0)),
        runs_allowed_home = as.numeric(ifelse(.data$home_game == 1, .data$away_team_score,0)),
        runs_scored_away = as.numeric(ifelse(.data$away_game == 1, .data$away_team_score,0)),
        runs_allowed_away = as.numeric(ifelse(.data$away_game == 1, .data$home_team_score,0)),
        school = school_name) %>%
    ...
    df = df %>%
      dplyr::mutate(
        home_game = dplyr::case_when(id == 749 & date %in% c("06/02/2023","06/03/2023",
                                                      "06/04/2023","06/10/2023","06/11/2023") ~ as.numeric(1),
                              .data$home_team_id == id & is.na(.data$neutral_site) ~ as.numeric(1),
                              TRUE ~ as.numeric(0)),
        away_game = as.numeric(ifelse(.data$away_team_id == id | !is.na(.data$neutral_site), 1, 0)),
        runs_scored_home = as.numeric(ifelse(.data$home_game == 1, .data$home_team_score,0)),
        runs_allowed_home = as.numeric(ifelse(.data$home_game == 1, .data$away_team_score,0)),
    ...
    if (type == "division") {
      ...
      dfa = dfa %>% 
        dplyr::mutate(
          ...
          RPGR = (.data$runs_scored_away+.data$runs_allowed_away)/(.data$away_game),
          TM = round(mean(teams$n),0),
          base_pf = (.data$RPGH*.data$TM)/((.data$TM-1)*.data$RPGR),
          home_game_adj = ifelse(.data$base_pf > 1, 
                                 .data$base_pf-(abs(.data$base_pf-1)*(.data$home_game/(.data$home_game+.data$away_game))), 
                                 .data$base_pf+(abs(.data$base_pf-1)*(.data$home_game/(.data$home_game+.data$away_game)))),
          final_pf = (1-(1-.data$home_game_adj)*.95),
